...
export const WORKFLOW_WORKSTREAM_SCHEMA = "@tsuuanmi/internet-workflow-workstream";
...
function assertImport(value, childRunId) {
    if (!isRecord(value))
        throw new Error("invalid workflow continuation import");
...
function assertContinuation(value, runIds) {
    if (!isRecord(value))
        throw new Error("invalid workflow continuation link");
    ...
    if (!Array.isArray(value.imports))
        throw new Error("invalid workflow continuation imports");
...
export function parseWorkflowWorkstream(value) {
    if (!isRecord(value) ||
    ...
    if (value.title !== undefined)
        assertText(value.title, "workflow Workstream title");
    if (!Array.isArray(value.runIds))
        throw new Error("invalid workflow Workstream run ids");
    const runIds = new Set();
    for (const runId of value.runIds) {
        assertHex(runId, 32, "workflow Workstream run id");
        if (runIds.has(runId))
            throw new Error(`duplicate workflow Workstream run ${runId}`);
        runIds.add(runId);
    }
    if (runIds.size === 0)
        throw new Error("workflow Workstream requires at least one run");
    if (!Array.isArray(value.continuations))
        throw new Error("invalid workflow Workstream continuations");
    const children = new Set();
    for (const continuation of value.continuations) {
        assertContinuation(continuation, runIds);
        const childRunId = continuation.childRunId;
        if (children.has(childRunId))
            throw new Error(`duplicate workflow continuation child run ${childRunId}`);
