...
import { WORKFLOW_EXTERNAL_EVENT_SCHEMA, WORKFLOW_EXTERNAL_EVENT_WAIT_SCHEMA, } from "#internet/workflow/awaitables/types";
...
function assertPrivateFile(path, label) {
    ...
    if (!stat.isFile())
        throw new WorkflowExternalEventStoreError(`${label} is not a regular file`);
...
function waitMatchesEvent(wait, event) {
    return (wait.runId === event.runId &&
...
export class WorkflowExternalEventStore {
    ...
    ingest(input, now = Date.now) {
        ...
        if (existing !== undefined) {
            ...
            if (canonicalJson(normalizedExisting) !== canonicalJson(next)) {
                throw new WorkflowExternalEventStoreError(`workflow ExternalEvent ${input.source}:${input.sourceEventId} was replayed with conflicting content`);
            }
            return existing;
        }
    ...
    ensureWait(runId, causedBy, contract, now = Date.now) {
        ...
        const conflicting = this.listWaits(runId).find((wait) => wait.causedBy.artifactId === causedBy.artifactId &&
        ...
        if (conflicting !== undefined) {
            throw new WorkflowExternalEventStoreError(`workflow Need ${causedBy.artifactId} already owns ExternalEvent wait ${conflicting.waitId} with a different contract`);
        }
    ...
    getEvent(runId, eventId) {
        ...
        try {
            ...
            throw new WorkflowExternalEventStoreError(`workflow ExternalEvent ${eventId} is invalid: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    getWait(runId, waitId) {
        ...
        try {
            ...
            throw new WorkflowExternalEventStoreError(`workflow ExternalEvent wait ${waitId} is invalid: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    listEvents(runId) {
    ...
    updateWait(runId, waitId, expectedRevision, mutate) {
        ...
        if (current.revision !== expectedRevision) {
            throw new WorkflowExternalEventStoreError(`workflow ExternalEvent wait ${waitId} revision conflict: expected ${expectedRevision}, current ${current.revision}`);
        }
...
//# sourceMappingURL=external-event-store.js.map