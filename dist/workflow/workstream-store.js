...
function assertPrivateFile(path, label) {
    ...
    if (!stat.isFile())
        throw new WorkflowWorkstreamStoreError(`${label} is not a regular file`);
...
export class WorkflowWorkstreamStore {
    ...
    get(workstreamId) {
        ...
        try {
            ...
            throw new WorkflowWorkstreamStoreError(`workflow Workstream ${workstreamId} is invalid: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    list() {
    ...
    update(workstreamId, expectedRevision, mutate) {
        ...
        if (current.revision !== expectedRevision) {
            throw new WorkflowWorkstreamStoreError(`workflow Workstream ${workstreamId} revision conflict: expected ${expectedRevision}, current ${current.revision}`);
        }
...
//# sourceMappingURL=workstream-store.js.map