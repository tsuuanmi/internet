...
function assertPrivateFile(path, label) {
    ...
    if (!stat.isFile())
        throw new WorkflowRunStoreError(`${label} is not a regular file`);
...
export class WorkflowRunStore {
    ...
    create(run) {
        ...
        if (existsSync(path))
            throw new WorkflowRunStoreError(`workflow run ${run.runId} already exists`);
    ...
    get(runId) {
        ...
        try {
            ...
            throw new WorkflowRunStoreError(`workflow run ${runId} is invalid: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    list() {
    ...
    update(runId, expectedRevision, mutate) {
        const current = this.get(runId);
        if (current === undefined)
            throw new WorkflowRunStoreError(`workflow run ${runId} does not exist`);
        if (current.revision !== expectedRevision) {
            throw new WorkflowRunStoreError(`workflow run ${runId} revision conflict: expected ${expectedRevision}, current ${current.revision}`);
        }
        const next = mutate(current);
        if (next.runId !== current.runId)
            throw new WorkflowRunStoreError("workflow run id cannot change");
