...
import { workflowArtifactId, workflowArtifactPayloadHash, workflowInputBundleId, } from "#internet/workflow/kernel/identity";
import { WORKFLOW_ARTIFACT_LINEAGE_RELATIONS, WORKFLOW_ARTIFACT_SCHEMA, WORKFLOW_INPUT_BUNDLE_SCHEMA, WORKFLOW_RUN_LIFECYCLES, WORKFLOW_RUN_SCHEMA, WORKFLOW_SIDE_EFFECT_CLASSES, WORKFLOW_WORK_ITEM_SCHEMA, WORKFLOW_WORK_ITEM_STATES, } from "#internet/workflow/kernel/types";
...
function assertUniqueVersionRefs(value, label) {
...
function assertDefinitions(value) {
    if (!isRecord(value))
        throw new Error("invalid workflow definition bindings");
...
function assertArtifactProducer(value) {
    ...
    if (value.kind === "external_import") {
        if (!isRecord(value.source))
            throw new Error("invalid workflow external-import source");
...
function assertArtifactRef(value) {
    if (!isRecord(value))
        throw new Error("invalid workflow artifact reference");
...
function assertArtifactRefs(value) {
    if (!Array.isArray(value))
        throw new Error("invalid workflow input artifacts");
    const seen = new Set();
    for (const item of value) {
        assertArtifactRef(item);
        const key = `${item.runId}:${item.artifactId}`;
        if (seen.has(key))
            throw new Error(`duplicate workflow input artifact ${key}`);
        seen.add(key);
    }
}
function assertLineage(value) {
    if (!Array.isArray(value))
        throw new Error("invalid workflow artifact lineage");
    const seen = new Set();
    for (const item of value) {
        ...
        if (seen.has(key))
            throw new Error(`duplicate workflow artifact lineage ${key}`);
        seen.add(key);
    }
}
...
function assertInputFacts(value) {
    if (!Array.isArray(value))
        throw new Error("invalid workflow input facts");
    const names = new Set();
    for (const item of value) {
        ...
        if (names.has(item.name))
            throw new Error(`duplicate workflow input fact ${item.name}`);
        names.add(item.name);
    }
}
export function parseWorkflowRun(value) {
    ...
    if (typeof value.lifecycle !== "string" ||
        !WORKFLOW_RUN_LIFECYCLES.includes(value.lifecycle))
...
export function parseWorkflowArtifact(value) {
    ...
    if (value.inputBundleId !== undefined)
        assertHex(value.inputBundleId, 64, "workflow artifact input bundle id");
    ...
    if (!isTimestamp(value.createdAt))
        throw new Error("invalid workflow artifact timestamp");
    ...
    if (value.artifactId !== expectedId)
        throw new Error("workflow artifact id does not match content identity");
    return value;
}
export function parseWorkflowWorkItem(value) {
    ...
    if (typeof value.sideEffect !== "string" ||
        !WORKFLOW_SIDE_EFFECT_CLASSES.includes(value.sideEffect))
        throw new Error("invalid workflow work item side-effect class");
    if (typeof value.state !== "string" || !WORKFLOW_WORK_ITEM_STATES.includes(value.state))
        throw new Error("invalid workflow work item state");
    if (value.inputBundleId !== undefined)
        assertHex(value.inputBundleId, 64, "workflow work item input bundle id");
    ...
    if (value.authorityRef !== undefined)
        assertText(value.authorityRef, "workflow work item authority reference");
    if (value.budgetRef !== undefined)
        assertText(value.budgetRef, "workflow work item budget reference");
    ...
    for (const artifactId of value.resultArtifactIds)
        assertHex(artifactId, 64, "workflow work item result artifact id");
    assertStringList(value.receiptIds, "workflow work item receipt id");
    if (!isTimestamp(value.createdAt) || !isTimestamp(value.updatedAt))
        throw new Error("invalid workflow work item timestamps");
    return value;
}
export function parseWorkflowInputBundle(value) {
    ...
    if (!isTimestamp(value.createdAt))
        throw new Error("invalid workflow input bundle timestamp");
    ...
    if (value.bundleId !== expectedId)
        throw new Error("workflow input bundle id does not match content identity");
    return value;
}
