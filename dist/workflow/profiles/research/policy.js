...
import { parseWorkflowAcceptanceCriteriaPayload, parseWorkflowCriterionAssessmentPayload, parseWorkflowFindingPayload, WORKFLOW_SEMANTIC_ARTIFACT_TYPES, } from "#internet/workflow/semantic/index";
function positiveInteger(value, label) {
    if (!Number.isSafeInteger(value) || value < 1)
        throw new Error(`${label} must be a positive integer`);
    return value;
}
function currentArtifacts(run, artifacts, inputBundles) {
...
function refsOf(artifacts, types) {
    return artifacts
        .filter((artifact) => types.includes(artifact.type))
        .map((artifact) => ({ runId: artifact.runId, artifactId: artifact.artifactId }));
}
function researchRoundId(context) {
    if (context.need.requestOwner.kind !== "research_round")
        return undefined;
    const round = Number(context.need.requestOwner.id);
    if (!Number.isSafeInteger(round) || round < 1)
        throw new Error("research round Need has an invalid round id");
    return round;
}
export function createWorkflowResearchPolicy(artifacts, inputBundles, awaitables, options = {}) {
    const maxRounds = positiveInteger(options.maxRounds ?? 2, "research maxRounds");
    const roundWait = options.roundWait ?? { kind: "timer", delayMs: 60 * 60_000 };
    if (roundWait.kind === "timer")
        positiveInteger(roundWait.delayMs, "research round Timer delay");
    else if (roundWait.eventType.trim() === "")
        throw new Error("research round ExternalEvent type is required");
    return {
        materializeNeed(context) {
            const round = researchRoundId(context);
            if (round !== undefined) {
                if (round > maxRounds)
                    throw new Error("research round exceeds configured maximum");
        ...
        readiness(context) {
            ...
            if (context.need.requestOwner.kind === "research_report") {
                ...
                const hasCriteria = current.some((artifact) => artifact.type === WORKFLOW_SEMANTIC_ARTIFACT_TYPES.acceptanceCriteria);
        ...
        convergence(run) {
            ...
            const criteriaArtifact = current.find((artifact) => artifact.type === WORKFLOW_SEMANTIC_ARTIFACT_TYPES.acceptanceCriteria);
            ...
            if (criterion === undefined)
                throw new Error("research AcceptanceCriteria is missing research-report");
