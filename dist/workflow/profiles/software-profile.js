...
function userConfirmationReasons(draft) {
    const reasons = [];
...
function localConfirmationReasons(draft) {
    const reasons = [];
    ...
    if (draft.authority?.repositoryMutation?.value === true &&
...
export const SOFTWARE_WORKFLOW_PROFILE = {
    ...
    preflightAdmission(draft) {
        ...
        else if (!fullSha(baseRevision))
            errors.push("target.baseRevision must be a full lowercase Git SHA");
        if (repositoryMutation === undefined)
            unresolved.push("authority.repositoryMutation");
        else if (repositoryMutation.value !== true)
            errors.push("software_change requires repository mutation authority");
        if (draft.source.provenance === "local_interpreted") {
