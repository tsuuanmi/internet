...
import { WORKFLOW_EXTERNAL_EVENT_SCHEMA, WORKFLOW_EXTERNAL_EVENT_WAIT_SCHEMA, WORKFLOW_EXTERNAL_EVENT_WAIT_STATES, WORKFLOW_TIMER_SCHEMA, WORKFLOW_TIMER_STATES, } from "#internet/workflow/awaitables/types";
...
function versionRef(value, label) {
...
export function parseWorkflowTimer(value) {
    ...
    if (timer.causedBy.runId !== timer.runId) {
    ...
    if (typeof timer.state !== "string" || !WORKFLOW_TIMER_STATES.includes(timer.state)) {
        throw new Error("invalid workflow Timer state");
    }
    if (timer.firedAt !== undefined)
        timestamp(timer.firedAt, "workflow Timer firedAt");
...
export function parseWorkflowExternalEventWait(value) {
    ...
    if (wait.causedBy.runId !== wait.runId) {
    ...
    if (wait.payloadSchema !== undefined)
        versionRef(wait.payloadSchema, "workflow ExternalEvent wait payload schema");
    if (typeof wait.state !== "string" ||
        !WORKFLOW_EXTERNAL_EVENT_WAIT_STATES.includes(wait.state)) {
        throw new Error("invalid workflow ExternalEvent wait state");
    }
    if (wait.matchedEventId !== undefined)
        hex(wait.matchedEventId, 32, "workflow ExternalEvent matched event id");
...
export function parseWorkflowExternalEvent(value) {
