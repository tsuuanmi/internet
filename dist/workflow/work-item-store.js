...
function assertPrivateFile(path, label) {
    ...
    if (!stat.isFile())
        throw new WorkflowWorkItemStoreError(`${label} is not a regular file`);
    if (process.platform !== "win32" && (stat.mode & 0o077) !== 0)
        throw new WorkflowWorkItemStoreError(`${label} permissions must be 0600`);
}
function assertAppendOnly(current, next, label) {
...
export class WorkflowWorkItemStore {
    ...
    get(runId, workItemId) {
        ...
        try {
            ...
            throw new WorkflowWorkItemStoreError(`workflow work item ${workItemId} is invalid: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    list(runId) {
    ...
    update(runId, workItemId, expectedRevision, mutate) {
        ...
        if (current.revision !== expectedRevision) {
            throw new WorkflowWorkItemStoreError(`workflow work item ${workItemId} revision conflict: expected ${expectedRevision}, current ${current.revision}`);
        }
...
//# sourceMappingURL=work-item-store.js.map