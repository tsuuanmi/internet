...
export class WorkflowExecutionManager {
    ...
    heartbeat(runId, executionId, ownerInstanceId) {
        ...
        if (Date.parse(current.leaseUntil) <= this.now()) {
            fenceWorkflowExecution(this.dependencies.executions, current, this.now, "LEASE_EXPIRED", "workflow execution lease expired before heartbeat");
    ...
    async execute(run, workItemId, ownerInstanceId, signal) {
        ...
        try {
            ...
            fenceWorkflowExecution(this.dependencies.executions, execution, this.now, "WORK_ITEM_CLAIM_CONFLICT", "work item claim failed");
            throw error;
        }
        try {
            ...
            failWorkflowExecution(this.dependencies.executions, current, this.now, "EXECUTOR_FAILURE", error instanceof Error ? error.message : String(error), false);
    ...
    async reconcileExpiredExecution(execution, signal) {
        ...
        if (item === undefined || bundle === undefined) {
            failWorkflowExecution(this.dependencies.executions, execution, this.now, "MISSING_EXECUTION_CONTEXT", "execution context is missing", false);
            return;
        }
        const capability = this.dependencies.capabilities.resolve(execution.capability);
        if (capability.sideEffect === "READ_ONLY") {
            fenceWorkflowExecution(this.dependencies.executions, execution, this.now, "LEASE_EXPIRED", "read-only execution lease expired");
        ...
        if (executor.reconcile === undefined) {
            failWorkflowExecution(this.dependencies.executions, execution, this.now, "UNCERTAIN_EXTERNAL_MUTATION", "mutation execution lease expired without a reconciliation contract", false);
        ...
        if (run === undefined)
            throw new Error(`workflow run ${execution.runId} does not exist`);
        const result = await executor.reconcile({ run, workItem: item, execution, inputBundle: bundle }, signal);
        if (result === undefined) {
            fenceWorkflowExecution(this.dependencies.executions, execution, this.now, "RECONCILED_ABSENT", "external mutation was not observed during reconciliation");
            setWorkflowWorkItemReady(this.dependencies.workItems, execution.runId, execution.workItemId, this.now);
            return;
        }
