...
import { parseWorkflowSemanticExecutionResult, } from "#internet/workflow/semantic/index";
...
function assertPrivateFile(path, label) {
    ...
    if (!stat.isFile())
        throw new WorkflowExecutionResultStoreError(`${label} is not a regular file`);
    if (process.platform !== "win32" && (stat.mode & 0o077) !== 0)
        throw new WorkflowExecutionResultStoreError(`${label} permissions must be 0600`);
}
function assertExecutionBinding(execution, result) {
...
export class WorkflowExecutionResultStore {
    ...
    create(execution, resultValue) {
        ...
        if (existsSync(path)) {
            ...
            if (canonicalJson(current) !== canonicalJson(result))
                throw new WorkflowExecutionResultStoreError(`workflow result ${execution.executionId} conflicts with persisted result`);
            return current;
        }
    ...
    get(runId, executionId) {
        ...
        try {
            ...
            throw new WorkflowExecutionResultStoreError(`workflow execution result ${executionId} is invalid: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    list(runId) {
