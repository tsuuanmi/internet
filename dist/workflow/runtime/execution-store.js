...
import { WORKFLOW_EXECUTION_SCHEMA, WORKFLOW_EXECUTION_STATES, } from "#internet/workflow/runtime/types";
...
export function parseWorkflowExecution(value) {
    if (typeof value !== "object" || value === null || Array.isArray(value))
    ...
    if (typeof execution.state !== "string" ||
        !WORKFLOW_EXECUTION_STATES.includes(execution.state))
    ...
    if (execution.finishedAt !== undefined)
        assertTimestamp(execution.finishedAt, "workflow execution finishedAt");
    if (execution.failure !== undefined)
        assertFailure(execution.failure);
    ...
    if (["SUCCEEDED", "FAILED", "FENCED"].includes(execution.state) && execution.finishedAt === undefined)
...
function assertPrivateFile(path, label) {
    ...
    if (!stat.isFile())
        throw new WorkflowExecutionStoreError(`${label} is not a regular file`);
...
export class WorkflowExecutionStore {
    ...
    get(runId, executionId) {
        ...
        try {
            ...
            throw new WorkflowExecutionStoreError(`workflow execution ${executionId} is invalid: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    list(runId) {
    ...
    update(runId, executionId, expectedRevision, mutate) {
        ...
        if (current.revision !== expectedRevision)
            throw new WorkflowExecutionStoreError(`workflow execution ${executionId} revision conflict: expected ${expectedRevision}, current ${current.revision}`);
        ...
        for (const [label, before, after] of [
            ...
            if (before !== after)
                throw new WorkflowExecutionStoreError(`workflow execution ${label} cannot change`);
        }
