...
export function defineBrowserChatTool(manager, timeoutMs, allowed) {
    return defineTool({
        name: "browser_chat",
        description: "Ask ChatGPT or Gemini through a logged-in browser. ChatGPT durably resumes one native conversation per current DSH session. Use when you specifically need a web-model response or want multiple turns in the same ChatGPT conversation.",
        parameters: {
            model: {
                type: "string",
                required: true,
