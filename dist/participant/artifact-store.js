...
export const WEBSITE_PARTICIPANT_ARTIFACT_SCHEMA = "@tsuuanmi/internet-website-participant-artifact";
...
function identityHash(value, name) {
    if (value.trim() === "")
        throw new WebsiteParticipantArtifactStoreError(`${name} must not be empty`);
...
export function parseWebsiteParticipantArtifact(value) {
    ...
    if (value.provider !== provider)
        throw new Error("website participant provider does not match account");
    if (value.mode !== "chat" && value.mode !== "research")
        throw new Error("invalid website participant mode");
    ...
    if (typeof value.url !== "string" || value.url.trim() === "")
        throw new Error("invalid website participant URL");
    ...
    if (value.artifactId !== expectedId)
        throw new Error("website participant artifact id does not match identity");
    return value;
}
export class WebsiteParticipantArtifactStore {
    ...
    create(input) {
        if (input.prompt.trim() === "")
            throw new WebsiteParticipantArtifactStoreError("prompt must not be empty");
        if (input.text.trim() === "")
            throw new WebsiteParticipantArtifactStoreError("result text must not be empty");
        if (input.url.trim() === "")
            throw new WebsiteParticipantArtifactStoreError("result URL must not be empty");
        ...
        if (current !== undefined) {
            ...
            if (current.textHash !== textHash ||
                ...
                throw new WebsiteParticipantArtifactStoreError("website participant logical request completed with conflicting result content");
            }
            return current;
        }
        const artifact = {
    ...
    readForRequest(ownerSessionId, accountId, logicalRequestId, mode) {
    ...
    read(ownerSessionId, artifactId) {
    ...
    readText(ownerSessionId, artifactId, options) {
