...
export class BrowserManager {
    ...
    async captureLoginState(accountId, loginEnv) {
    ...
    async startRemoteLogin(accountId) {
        ...
        session = await RemoteLoginSession.start({
            ...
            finalize: (loginEnv, profileReady) => this.runAccountExclusive(accountId, async () => {
                await profileReady;
    ...
    async persistLoginProfile(accountId, loginEnv) {
        const provider = this.provider(accountId);
            ...
            if (request.requestKey !== undefined) {
                ...
                if (existing !== undefined) {
                    ...
                    if (reconciliation === "AMBIGUOUS") {
                        throw new InternetError("provider_reconciliation_failed", "provider conversation no longer matches the durable workflow turn receipt; inspect before retrying");
                    }
