...
describe("workflow graph reducer", () => {
	...
	it("fences a failed execution and retries only the same logical node", () => {
		...
		const recovering = recoverWorkflowNode(
			started,
			node.nodeId,
			"exec-1",
			"FENCED",
			{ class: "PROVIDER", code: "STALL_TIMEOUT", message: "stalled", retry: "RECREATE_SESSION", at: now },
			{ action: "RECREATE_SESSION", attempt: 2, maxAttempts: 3 },
		);
		expect(recovering.nodes[node.nodeId]?.state).toBe("RECOVERING");
		expect(recovering.nodes[node.nodeId]?.execution?.state).toBe("FENCED");
		const retried = retryWorkflowNode(recovering, node.nodeId, execution("exec-2", 2));
		expect(retried.nodes[node.nodeId]?.state).toBe("RUNNING");
	...
	it("rejects late completion from a fenced execution", () => {
		...
		const recovering = recoverWorkflowNode(
			...
			"exec-old",
			"ORPHANED",
			{ class: "TRANSPORT", code: "EXECUTION_ORPHANED", message: "owner lost", retry: "IMMEDIATE", at: now },
			{ action: "RECONCILE", attempt: 2, maxAttempts: 3 },
		);
