...
export class WorkflowJobStore {
	...
	list(): readonly WorkflowJob[] {
		...
		if (!lstatSync(this.jobsDir).isDirectory()) throw new WorkflowJobStoreError("workflow jobs path is not a directory");
	...
	update(jobId: string, expectedRevision: number, mutate: (current: WorkflowJob) => WorkflowJob): WorkflowJob {
		...
		if (next.revision !== current.revision + 1) throw new WorkflowJobStoreError("workflow job revision must increment by one");
...
export function parseWorkflowJob(value: unknown): WorkflowJob {
	...
	if (typeof value.ownerSessionId !== "string" || value.ownerSessionId.trim() === "") throw new Error("invalid workflow owner");
	if (typeof value.objective !== "string" || value.objective.trim() === "") throw new Error("invalid workflow objective");
	if (typeof value.repository !== "string" || value.repository.trim() === "") throw new Error("invalid workflow repository");
...
function assertPrivateFile(path: string, label: string): void {
	...
	if (process.platform !== "win32" && (stat.mode & 0o077) !== 0) throw new WorkflowJobStoreError(`${label} permissions must be 0600`);
}
...
function assertAccountRouting(value: unknown): void {
	...
	if (!isAccountId(first) || !isAccountId(second) || first === second) throw new Error("invalid workflow thinker routing");
...
function assertHandoffReceipts(value: unknown): void {
	...
	for (const item of value) {
		...
		if (typeof item.source !== "string" || item.source.trim() === "" || !isAccountId(item.recipient)) throw new Error("invalid handoff identity");
		if (!isPositiveInteger(item.sequence) || typeof item.payloadHash !== "string" || !/^[0-9a-f]{64}$/u.test(item.payloadHash))
...
function assertPullRequest(value: unknown): void {
	...
	if (!isRecord(value) || typeof value.repository !== "string" || !isPositiveInteger(value.number)) throw new Error("invalid pull request receipt");
	if (typeof value.url !== "string" || !/^https:\/\/github\.com\//u.test(value.url)) throw new Error("invalid pull request URL");
	if (typeof value.base !== "string" || typeof value.head !== "string" || !isFullSha(value.headSha)) throw new Error("invalid pull request identity");
}

function assertCiReceipt(value: unknown): void {
	if (value === undefined) return;
	if (!isRecord(value) || typeof value.repository !== "string" || !isPositiveInteger(value.number)) throw new Error("invalid CI receipt");
	if (typeof value.url !== "string" || !/^https:\/\/github\.com\//u.test(value.url) || !isFullSha(value.headSha)) throw new Error("invalid CI identity");
	if (typeof value.status !== "string" || !(WORKFLOW_CI_STATUSES as readonly string[]).includes(value.status)) throw new Error("invalid CI status");
	if (!isTimestamp(value.checkedAt)) throw new Error("invalid CI timestamp");
}

function assertMergeAuthorization(value: unknown): void {
	if (value === undefined) return;
	if (!isRecord(value) || typeof value.repository !== "string" || !isPositiveInteger(value.number)) throw new Error("invalid merge authorization");
	if (typeof value.url !== "string" || typeof value.head !== "string" || !isFullSha(value.headSha)) throw new Error("invalid merge authorization identity");
	if (!isPositiveInteger(value.reviewCycle) || !isTimestamp(value.authorizedAt) || typeof value.authorizedByOwnerSessionId !== "string")
		throw new Error("invalid merge authorization fields");
}

function assertMergeReceipt(value: unknown): void {
	if (value === undefined) return;
	if (!isRecord(value) || typeof value.repository !== "string" || !isPositiveInteger(value.number)) throw new Error("invalid merge receipt");
	if (typeof value.url !== "string" || !isFullSha(value.headSha) || !isFullSha(value.mergedSha) || value.executorAccountId !== "chatgpt-writer" || !isTimestamp(value.mergedAt))
		throw new Error("invalid merge receipt fields");
}

function assertPendingAction(value: unknown): void {
	if (value === undefined) return;
	if (!isRecord(value) || typeof value.kind !== "string" || !(WORKFLOW_PENDING_ACTION_KINDS as readonly string[]).includes(value.kind))
	...
	if (value.nodeId !== undefined && (typeof value.nodeId !== "string" || value.nodeId.trim() === "")) throw new Error("invalid pending action node");
...
function assertEvent(value: unknown): void {
	if (value === undefined) return;
	if (!isRecord(value) || typeof value.type !== "string" || !["INTERNAL", "PROGRESS", "ACTION_REQUIRED"].includes(String(value.class)) || !isTimestamp(value.at))
