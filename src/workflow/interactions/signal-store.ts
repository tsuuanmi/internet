...
function assertPrivateFile(path: string, label: string): void {
	...
	if (!stat.isFile()) throw new WorkflowExternalSignalStoreError(`${label} is not a regular file`);
	if (process.platform !== "win32" && (stat.mode & 0o077) !== 0) {
		throw new WorkflowExternalSignalStoreError(`${label} permissions must be 0600`);
	}
}

function parse(value: unknown): WorkflowExternalSignal {
	if (
		typeof value !== "object" ||
		value === null ||
		Array.isArray(value) ||
		(value as { schema?: unknown }).schema !== WORKFLOW_EXTERNAL_SIGNAL_SCHEMA ||
		(value as { version?: unknown }).version !== 1
	) {
		throw new WorkflowExternalSignalStoreError("unsupported workflow external signal schema");
	}
	const signal = value as WorkflowExternalSignal;
...
export class WorkflowExternalSignalStore {
	...
	create(
		input: WorkflowExternalSignalInput,
		...
		const next: WorkflowExternalSignal = {
		...
		if (existing !== undefined) {
			if (
				...
				throw new WorkflowExternalSignalStoreError(
					`workflow external signal request ${input.requestId} was replayed with conflicting content`,
				);
			}
			return existing;
		}
	...
	get(runId: string, signalId: string): WorkflowExternalSignal | undefined {
		...
		try {
			...
			throw new WorkflowExternalSignalStoreError(
				`workflow external signal ${signalId} is invalid: ${error instanceof Error ? error.message : String(error)}`,
			);
		}
	}
