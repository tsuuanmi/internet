...
export async function runGitCommand(cwd: string, args: readonly string[], signal: AbortSignal): Promise<string> {
	return new Promise((resolve, reject) => {
		...
		child.once("close", (code) => {
			...
			settle(() =>
				reject(new WorkflowRepositoryError(detail === "" ? `git exited with status ${String(code)}` : detail)),
			);
		});
	});
}
...
export async function resolveWorkflowRepository(
	...
	source = "/workflow",
): Promise<WorkflowRepositoryContext> {
	try {
	...
	if (remote === undefined) {
		throw new WorkflowRepositoryError(
			`${source} could not select an upstream remote. Configure the current branch's remote or use an origin remote (found: ${remotes.join(", ")}).`,
		);
	}
