...
function receiptId(accountId: AccountId, sessionHash: string, requestKeyHash: string): string {
	return hashProviderTurnText(`${accountId}\0${sessionHash}\0${requestKeyHash}`);
}

export function providerTurnReceiptId(accountId: AccountId, sessionId: string, requestKey: string): string {
...
export function parseProviderTurnReceipt(value: unknown): ProviderTurnReceipt {
	...
	const expectedId = receiptId(value.accountId as AccountId, value.sessionHash, value.requestKeyHash);
