...
async function attachedChatGptPromptText(composer: Locator): Promise<string> {
...
async function verifyChatGptPromptAttached(composer: Locator, prompt: string): Promise<void> {
...
export async function chatgptSelectThinkingLevel(page: Page, level: ChatGptThinkingLevel): Promise<void> {
	...
	try {
		...
		throw new InternetError(
			"provider_error",
			`ChatGPT effort menu did not become visible${error instanceof Error ? ` (${error.message})` : ""}`,
		);
	}
	...
	const sliderAttached = await effortSlider
		...
		.then(() => true)
		.catch(() => false);
	const ready: "items" | "slider" = sliderAttached ? "slider" : "items";
	if (!sliderAttached) {
		...
		if (labels.length !== expectedLabels.length || labels.some((label, index) => label !== expectedLabels[index])) {
			throw new InternetError(
				"provider_error",
				`ChatGPT effort menu exposed neither its reasoning slider nor the three reasoning choices` +
