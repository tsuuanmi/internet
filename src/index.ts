...
const BROWSER_TEAM_GUIDANCE = [
	...
	"The default profile runs two hidden turns per provider, allowing independent child-session debates to overlap at ChatGPT or Gemini. Set maxConcurrentTurnsPerProvider to one if provider policy or account-state acceptance requires it; same-session turns, visible calls, login, and one team's dependent rounds remain ordered.",
